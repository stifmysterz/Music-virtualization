...
### 3.2 录制时按录制画质渲染 3D 层(用户 2026-09-25 决定,2026-09-26 修订为开关)

**修订(2026-09-26,看过实测后用户决定)**:本机集显上按录制画质渲染,录 4K 时 3D 层约 130–163 ms/帧(6–8 fps,改造前约 14–18 ms),录 1440p 约 60–73 ms,录 1080p 约 35–46 ms。因此做成 Tools 里的开关「录制时 3D:屏幕尺寸 / 跟随录制画质」(`record3DSharp`,存 `subremix_record3dsharp`),**默认屏幕尺寸**(与改造前一样快),需要锐利成片时手动打开;录制中不能切换。下面各条只在开关打开时生效。
